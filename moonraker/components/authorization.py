...
if TYPE_CHECKING:
    ...
    from .oauth2_device_flow import MoonrakerOAuth2DeviceFlow
...
class Authorization:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.oauth2_device_flow: Optional[MoonrakerOAuth2DeviceFlow] = None
        if config.get_prefix_sections('oauth2_device_flow '):
            self.oauth2_device_flow = self.server.load_component(
                config, "oauth2_device_flow", None
            )
        ...
        self.server.register_endpoint(
            ...
            self._handle_oauth2_device_flow_start,
            transports=TransportType.HTTP | TransportType.WEBSOCKET,
            auth_required=False
        )
        self.server.register_endpoint(
            "/access/oauth2/device/poll", RequestType.POST,
            self._handle_oauth2_device_flow_poll,
            transports=TransportType.HTTP | TransportType.WEBSOCKET,
            auth_required=False
        )
    ...
    async def _handle_info_request(self, web_request: WebRequest) -> Dict[str, Any]:
        ...
        if self.oauth2_device_flow is not None:
            oauth2_providers = self.oauth2_device_flow.get_provider_names()
        return {
    ...
    async def _handle_oauth2_device_flow_start(
        self, web_request: WebRequest
    ) -> Dict[str, Any]:
        if self.oauth2_device_flow is None:
            raise self.server.error("OAuth2 device flow not configured", 400)

        provider_name: str = web_request.get_str('provider')
        device_record = await self.oauth2_device_flow.initiate_device_flow(
            provider_name
        )
    ...
    async def _handle_oauth2_device_flow_poll(
        self, web_request: WebRequest
    ) -> Dict[str, Any]:
        if self.oauth2_device_flow is None:
            raise self.server.error("OAuth2 device flow not configured", 400)
        ...
        error, provider, username = await self.oauth2_device_flow.poll_token(
            device_code
        )
        ...
        if username not in self.users:
            user_info = UserInfo(
                ...
                source=f"oauth2_device_flow:{provider}"
            )
            self.users[username] = user_info
            await self._sync_user(username)
        else:
