...
from ..common import OAuth2DeviceRecord
...
class OAuth2DeviceFlowProvider:
    ...
    @classmethod
    def from_config(cls, config: ConfigHelper) -> OAuth2DeviceFlowProvider:
...
class MoonrakerOAuth2DeviceFlow:
    def __init__(self, config: ConfigHelper) -> None:
        self.server = config.get_server()

        # In-memory OAuth2 device flow storage
        self.device_records: Dict[str, OAuth2DeviceRecord] = {}

        # Load OAuth2 device flow providers
        self.providers: Dict[str, OAuth2DeviceFlowProvider] = {}
        prefix_sections = config.get_prefix_sections('oauth2_device_flow ')
        for section_name in prefix_sections:
            try:
                provider_config = config[section_name]
                provider = OAuth2DeviceFlowProvider.from_config(provider_config)
    ...
    async def initiate_device_flow(
        ...
        device_record = OAuth2DeviceRecord(
    ...
    async def _get_user_info(
        self, provider: OAuth2DeviceFlowProvider, access_token: str
    ) -> Dict[str, Any]:
    ...
    async def _validate_id_token(
        self, provider: OAuth2DeviceFlowProvider, id_token: str, access_token: str
    ) -> Dict[str, Any]:
        if not id_token:
            return {}
...
def load_component(config: ConfigHelper) -> MoonrakerOAuth2DeviceFlow:
    return MoonrakerOAuth2DeviceFlow(config)
