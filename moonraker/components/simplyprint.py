...
class SimplyPrint(APITransport):
    ...
    def _update_job_progress(self) -> None:
        ...
        if "display_status" in self.printer_status and "progress" in self.printer_status["display_status"] and self.printer_status["display_status"]["progress"]:
            progress = self.printer_status["display_status"]["progress"]
        elif "virtual_sdcard" in self.printer_status:
            if "file_position" in self.printer_status["virtual_sdcard"]:
                file_position = self.printer_status["virtual_sdcard"]["file_position"]
                gcode_start_byte = self.cache.metadata.get('gcode_start_byte')
                gcode_end_byte = self.cache.metadata.get('gcode_end_byte')
                if gcode_start_byte and gcode_end_byte and (gcode_end_byte - gcode_start_byte) > 0:
                    progress = (file_position - gcode_start_byte) / (gcode_end_byte - gcode_start_byte)
