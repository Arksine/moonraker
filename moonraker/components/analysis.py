...
class GcodeAnalysis:
    ...
    async def component_init(self) -> None:
        ...
        self._register_gcode_processor()

    def _register_gcode_processor(self) -> None:
        # Initalize GCode Processor config.  It would be preferable to register
        # in the __init__() method, however we don't know if Klipper Estimator
        # is downloaded and executable until we fetch the version.
        proc_config: Dict[str, Any] = {
            ...
            "enabled": self.estimator_ready and self.auto_analyze
        }
        mdst = self.file_manger.get_metadata_storage()
        mdst.register_gcode_processor("klipper_estimator", proc_config)
