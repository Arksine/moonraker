...
class PrusaSlicer(BaseSlicer):
    def check_identity(self, data: str) -> bool:
        ...
        excluded_aliases = ("slic3r, slic3r prusa edition")
        # Default base pattern for PrusaSlicer and its forks
        pattern = r"; generated by (.+?) on \d{4}-\d{2}-\d{2} at \d{2}:\d{2}:\d{2}"
        match = re.search(pattern, data)
        if match is None:
            # Fall back to search by known alias
            for name, expr in known_aliases.items():
                match = re.search(expr, data)
                if match:
                    logging.info(
                        f"Detected PrusaSlicer fork via fallback: {name}"
                    )
...
SUPPORTED_SLICERS: List[Type[BaseSlicer]] = [
    Slic3rPE, Slic3r, PrusaSlicer, BambuStudio, Cura,
    Simplify3D, KISSlicer, IdeaMaker, IceSL, KiriMoto
]
SUPPORTED_DATA = [
    'gcode_start_byte',
    'gcode_end_byte',
